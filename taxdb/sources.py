...
STATE_BULK = [
    ...
    ("MD", "Maryland DLS local tax rates report",
     ...
     "income_payroll,property,other_levy"),
    ...
    ("FL", "Florida EDR county and municipal data",
     ...
     "lodging_meals,sales_use,other_levy"),
...
def _seed_scoped(conn, usps, name, url, stype, tier, note, cats=None):
...
def seed_catalog(conn):
    ...
    for usps, (name, url) in STATE_AGENCIES.items():
        ...
        n += 1
    for usps, name, url, stype, tier, note, cats in STATE_BULK:
        _seed_scoped(conn, usps, name, url, stype, tier, note, cats)
        n += 1
