...
def follow_targets(source_url, final_url, blob, depth, seed_hosts, name=None, state=None):
    ...
    for href, anchor in html_links(final_url, blob):
        if not allowed_host(urlparse(href).hostname, seed_hosts, name=name, state=state):
            continue
        if should_follow(href, anchor, depth) or looks_relevant(source_url):
...
def item_seeds(conn, client, settings, geoid, category, name, state, kind):
    """Catalog URLs first, then web search for the site and its tax PDFs."""
...
def crawl_item_legacy(conn, client, settings, run_id, geoid, category, name, state):
    ...
    seed_urls = item_seeds(conn, client, settings, geoid, category, name, state, kind)
